...
#| echo: false
#| out.width: "100%"
knitr::include_graphics("/Users/Noah/Desktop/ESS330 Spring25/Lab4/img/LTER-Network-Logo-2017.jpg")
...
**Answer:** First I had to clean up the data, to filter only Coastal giant salamander, and then drop all N/A values in the unittype and section columns. Then I made a cont table to run the chi-square test. My x-squared value was = 200.71, my degrees of freedom was = 5, and my p-value was = (2.2e-16). Such a low p-value means that I can reject the null hypothesis, meaning there is a statistical significant result that there is a direct relationship between channel and forest type for Coastal giant salamander abundance.

```{r}
coastal_giant_sal_clean <- and_vertebrates %>%  
  filter(species == "Coastal giant salamander") %>% 
  drop_na(unittype, section)

cont_table1 <- table(coastal_giant_sal_clean$section, coastal_giant_sal_clean$unittype)
```
...
**Answer:** First I filtered the data set to only include Coastal Giant salamander, in both clear cut and old growth forest types. I also dropped all N/A values for biomass.
```{r}
coastal_giant_sal_biomass <- and_vertebrates %>%
  filter(species == "Coastal giant salamander", 
         section %in% c("CC", "OG")) %>%
  drop_na(weight_g)
```

I then needed two vectors of the weights (biomass) in each separate forest section. I then performed a equal varience test on the two vectors. I got (p-value =1.439e-07), which means I have to reject the null hypothesis. The null hypothesis being: The true ratio of variances is equal to one. This means that the variances are not equal.

```{r}
cc_sal_weight <- coastal_giant_sal_clean %>%  
  filter(section == "CC") %>%  
  pull(weight_g)

og_sal_weight <- coastal_giant_sal_clean %>%  
  filter(section == "OG") %>% 
  pull(weight_g)

var.test(cc_sal_weight, og_sal_weight)
```
I can double check my work from the previous code by plotting the data, and see that it is right skewed. This means I will have to perform a transformation on the data.
```{r}
ggpubr::ggarrange(ggpubr::gghistogram(cc_sal_weight, main = "Clear Cut"), 
                  ggpubr::gghistogram(og_sal_weight, main = "Old Growth"))
```
Here I did another equal variance test, but took log transformation of the data, and got (p-value=0.005299). This isn't the best p-value, so I am going to plot the log transformed data in the next step to see if there is a normal distribution.
```{r}
var.test(log(log(cc_sal_weight)), log(log(og_sal_weight)))
```
After plotting the log transformed data, the distribution looks pretty normal, so we are going to go with it. (Also I ran a double log transformation on the clear cut and old growth vectors, and it skewed the data left, so a double log wouldn't work.)
```{r}
ggpubr::ggarrange(ggpubr::gghistogram(log(cc_sal_weight), main = "Clear Cut"), 
                  ggpubr::gghistogram(log(og_sal_weight), main = "Old Growth"))
```
Finally, I did a t test on the log transformed data, and got (p-value=2.2e-6). This means that we can rejust the null hypothesis, and conclude there is a statistical significant difference in biomass between clear cut and  old growth forests for the Coastal giant salamander. From our result, we can see that the mean biomass in clear cut forests is 0.252393 grams higher than biomass in old growth forests.
```{r}
t.test(log(coastal_giant_sal_clean$weight_g) ~ coastal_giant_sal_clean$section, var.equal = TRUE)
```
...
**Answer:** By using ?and_vertebrates, I found out that the two columns I need to use are length_1_mm for body length, and weight_g for body mass. First I need to clean the data, so I only have the data I want for this problem.
```{r}
trout1_clean <- and_vertebrates %>%  
  filter(species == "Cutthroat trout") %>% 
  drop_na(length_1_mm, weight_g)
```

Then I will create a histogram to visually represent the data.
```{r}
ggarrange(gghistogram(trout1_clean$length_1_mm, title = "Length"),
gghistogram(trout1_clean$weight_g, title = "Weight"))
```
Seeing the data visually, it is clear that it is right skewed. I'll then test to see if the data will fit a normal distribution with a shapiro test. I will have to slice the data, since apparently it only works on 5,000 or less observations.
As we can see from the above results, our (p-value = 2.2e-16) for both of our normality tests. This means that we reject the null hypothesis: The variable is normally distributed. This means the data is not normally distributed, so I will have to log transform the data.

```{r}
t1 <- trout1_clean %>% 
  slice_sample(n = 5000)

shapiro.test(t1$length_1_mm)
shapiro.test(t1$weight_g)
```
Here I plotted the log transformed data, and got a somewhat normal distribution, even though it looks double peaked. 
```{r}
ggarrange(
 gghistogram(log(trout1_clean$length_1_mm), title = "Length"), 
 gghistogram(log(trout1_clean$weight_g), title = "Weight") 
)
```
Next I will perform a correlation test on the log transformed variables, and from the results, we can see a very small p-value, and a correlation very close to one, showing that there is a strong correlation between body length and mass.
```{r}
cor.test(log(trout1_clean$length_1_mm), log(trout1_clean$weight_g))
```
...
trout1_clean %>%  
  ...
  ggscatter(x = 'log_length', 
            ...
            add = "loess")
```
After all of that, I can now try a spearman correlation test with the raw data, to see if there is still a strong correlation between body length and mass.
```{r}
cor.test(trout1_clean$length_1_mm,trout1_clean$weight_g, method = "spearman")
```
From Spearman's Correlation test, we can see there is still a low p-value and correlation value close to 1, showing that there is a correlation between body length and mass of observed cutthroat trout.
...
**Answer**
First I had to reorder the site data by latitude, otherwise it wouldn't plot it by latitiude. Then I made a simple box plot.
```{r}
pie_sites$site <- reorder(pie_sites$site, pie_sites$latitude)

ggplot(pie_sites, aes(x = site, y = size, color = site)) +
  ...
  theme_minimal() 
```
The ANOVA test above was conducted to examine the differences in crab carapace width among the three sites. The results showed a significant difference in mean carapace width between sites (F-value = 39.108), (p-value < 2.2e-16). Post-hoc Tukey’s HSD tests revealed that crabs at Site PIE had significantly larger carapace widths than those at Site DB and Site GTM.
...
```{r}
wtr_temp_model <- lm(size ~ water_temp_sd, data = pie_crab)

summary(wtr_temp_model)

```
After performing a linear regression analysis, the results indicated that annual variation in water temperature (water_temp_sd) had no significant effect on crab carapace width (F = 0.40, p = 0.53, R^2 = 0.001), suggesting that water_temp_sd has very little to do with determining carapace width.
...
```
```{r}
multi_model <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(multi_model)
```
After performing the correlation analysis, it revealed a strong positive correlation between latitude and air_temp_sd (r = 0.79), while water_temp_sd showed weak correlations with both predictors. The multiple linear regression indicated that latitude had a significant positive effect on crab carapace width (B = 0.559, p < 0.001), while air_temp_sd (p = 0.173) and water_temp_sd (p = 0.325) were not significant predictors. The R^2 value of (0.3466) explained that there was a 35% variation of carapace width of the entire model.
